...
const useZustandPaginationReducer = create(persist(redux(Pagination.reducer, {
  max: 0,
  min: 0,
  page: 0
}), {
...
export function usePaginationReducer({ size }: UsePaginationReducerProps) {
  const pagination = useZustandPaginationReducer((state) => state);
  const dispatch = useZustandPaginationReducer((state) => state.dispatch);
